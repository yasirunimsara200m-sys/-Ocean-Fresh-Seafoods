...
router.get('/', requireAdmin, (req, res) => {
    ...
    `).get();
        ...
        const pendingOrdersRow = db.prepare("SELECT COUNT(*) as pending_orders FROM orders WHERE order_status = 'pending'").get();
        const productsCountRow = db.prepare('SELECT COUNT(*) as total_products FROM products').get();
        const lowStockCountRow = db.prepare('SELECT COUNT(*) as low_stock FROM products WHERE in_stock = 0 OR stock_quantity <= 5').get();
        const unreadMessagesRow = db.prepare('SELECT COUNT(*) as unread_messages FROM messages WHERE is_read = 0').get();
